...
def collector(LOCAL_DIR):
	...
	return guts_togo, name_togo, lo_togo, hashes


def container(guts_togo, name_togo, lo_togo, hashes):
...
def upload(conn, t_name, data):
	cursor = conn.cursor()

	for row in contained:
		q = f"INSERT INTO {t_name} (note_tl, note_lo, note, hash) VALUES (%s, %s, %s, %s);"
...
if __name__=="__main__":
	loose = collector(LOCAL_DIR)
	contained = container(*loose)
	
	conn = initialize_connection(DB_USER, DB_PSWD, DB_NAME, DB_ADDR)
	t_name = table_support(conn)

	data = [(row.note_tl, row.note_lo, row.note, row.hash_id) for row in contained]
